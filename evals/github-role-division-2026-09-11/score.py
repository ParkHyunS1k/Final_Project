...
DATA = ROOT.parents[1] / "datasets/github-role-division-candidates-v1"
...
def score_case(label, response, task_ids):
    ...
    # 역할 분담 평가: 새 업무 제목은 표현이 달라 맞추지 않고, 지정된 담당자 계정이 맞는지만 센다.
    exp_people = Counter(c["after"]["person"] for c in expected if c["kind"] == "createTask" and c["after"]["person"])
    pred_people = Counter(c["person"] for c in predicted if c["kind"] == "createTask" and c["person"])
    exp_assign = {(c["task_id"], c["after"]["person"]) for c in expected if c["kind"] == "assignee"}
    pred_assign = {(c["task_id"], c["person"]) for c in predicted if c["kind"] == "assignee"}
    ...
    return {
        ...
        "create_expected": exp_keys[("createTask", None)],
        "create_predicted": pred_keys[("createTask", None)],
        "create_person_expected": sum(exp_people.values()),
        "create_person_predicted": sum(pred_people.values()),
        "create_person_matched": sum((exp_people & pred_people).values()),
        "assignee_expected": len(exp_assign),
        "assignee_predicted": len(pred_assign),
        "assignee_matched": len(exp_assign & pred_assign),
...
def aggregate(cases, records):
    ...
    return {
        ...
        "create_count_recall": ratio(min(s["create_predicted"], s["create_expected"]), s["create_expected"]),
        "create_cases": f"label {sum(1 for c in cases if c['create_expected'])} / pred {sum(1 for c in cases if c['create_predicted'])} / both {sum(1 for c in cases if c['create_expected'] and c['create_predicted'])}",
        "create_on_no_change_cases": sum(1 for c in cases if c["create_predicted"] and c["label_decision"] != "propose_changes"),
        "create_person": f"{s['create_person_matched']}/{s['create_person_expected']} (pred {s['create_person_predicted']})",
        "assignee": f"{s['assignee_matched']}/{s['assignee_expected']} (pred {s['assignee_predicted']})",
